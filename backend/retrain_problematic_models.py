#!/usr/bin/env python3
"""
Retrain 3 Problematic LGBM Models Using Unified Pipeline
...
This ensures all models use the same training pipeline with proper scaling.
"""
...
# Models to retrain with their category keys
PROBLEMATIC_MODELS = [
...
def create_training_data(category_key: str, display_name: str, n_samples: int = 100):
    """
    Create training dataset using compute_real_features

    This matches the exact pipeline used for all other models.
    """
...
def train_lgbm_model(category_key: str, display_name: str):
    """
    Train LGBM model using unified pipeline
    """
    logger.info(f"\n{'='*80}")
...
def main():
    ...
    logger.info("="*80)
    logger.info("RETRAINING 3 PROBLEMATIC LGBM MODELS")
    logger.info("Using unified feature_computer_full pipeline")
    logger.info("="*80)
    ...
    for category_key in PROBLEMATIC_MODELS:
        display_name = CATEGORY_DISPLAY_NAMES.get(category_key, category_key)

        try:
            # Train model
            model_data = train_lgbm_model(category_key, display_name)
            ...
            # Save model
            model_filename = f"{category_key}_LGBM_model.pkl"
    ...
    summary_path = backend_path / "retraining_problematic_summary.json"
