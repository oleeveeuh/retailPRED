#!/usr/bin/env python3
"""
Generate Weekly Predictions for 2026 for Retrained Models

This script generates predictions for ALL weeks in 2026
...
def generate_weekly_predictions_2026(model_info: dict):
    """Generate predictions for all weeks in 2026"""
    logger.info(f"\n{'='*80}")
    ...
    # Generate weekly predictions for 2026
    logger.info(f"\n  Generating weekly predictions for 2026...")

    # Start from first Friday of 2026
    start_date = datetime(2026, 1, 2)  # 2026-01-02 is a Friday
    ...
    logger.info(f"  Found {len(fridays)} Fridays in 2026")
    ...
    logger.info(f"    Unvalidated (future): {len(predictions) - validated}")
    ...
    # Delete existing 2026 predictions for this model
    cursor.execute("""
        DELETE FROM prediction_log
        WHERE model_name = ?
        AND prediction_date >= '2026-01-01'
        AND prediction_date <= '2026-12-31'
    """, (model_name,))

    deleted = cursor.rowcount
    if deleted > 0:
        logger.info(f"  Deleted {deleted} existing 2026 predictions")
...
def main():
    """Main function"""
    logger.info("="*80)
    logger.info("GENERATING WEEKLY PREDICTIONS FOR 2026")
    logger.info("="*80)
    ...
    for model_info in RETRAINED_MODELS:
        try:
            predictions = generate_weekly_predictions_2026(model_info)
            if predictions:
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"COMPLETE - Generated {total_added} weekly predictions for 2026")
    logger.info(f"{'='*80}")
    ...
    logger.info("\n✓ All retrained models now have complete 2025-2026 predictions!")
