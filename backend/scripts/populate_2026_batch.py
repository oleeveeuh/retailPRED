#!/usr/bin/env python3
"""
Generate 2026 predictions with batch inserts to avoid database locking
"""
...
def main():
    logger.info("=" * 80)
    logger.info("Starting 2026 Yearly Prediction Population (Batch Mode)")
    logger.info("=" * 80)
        ...
        for category in categories:
            for model_name in models:
                try:
                    ...
                    logger.info(f"  ✓ {category} - {model_name}: ${forecast_list[0]['predicted_value']:,.2f}")

                except Exception as e:
                    logger.error(f"  ✗ {category} - {model_name}: {str(e)[:100]}")
                    error_count += 1

        # Batch insert every 100 predictions to avoid memory issues
