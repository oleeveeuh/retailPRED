#!/usr/bin/env python3
"""
Pre-compute SHAP values for ALL predictions
Computes SHAP for LGBM, PatchTST, and TimesNet models across all categories
"""
...
def compute_and_save_shap(prediction_id):
    ...
    if not model_type:
        return False, f"Model {model_name} does not support SHAP"
...
def main():
    """Main function"""
    logger.info("=" * 80)
    logger.info("Pre-computing SHAP values for ALL predictions")
    logger.info("=" * 80)

    # Get all predictions that need SHAP values
    conn = db.get_connection()
    cursor = conn.cursor()

    # Get LGBM, PatchTST, and TimesNet predictions without SHAP
    cursor.execute("""
        SELECT id, model_name, prediction_date
        FROM prediction_log
        WHERE (model_name LIKE '%LGBM%'
           OR model_name LIKE '%PatchTST%'
           OR model_name LIKE '%TimesNet%')
        AND (shap_values IS NULL OR shap_values = '{}')
        ORDER BY id
    """)
    ...
    logger.info(f"Found {len(predictions)} predictions that need SHAP values")
    ...
    for pred in tqdm(predictions, desc="Computing SHAP values"):
        ...
        if success:
            ...
            if fail_count <= 5:  # Only show first 5 errors
                logger.warning(f"Prediction {prediction_id}: {message}")
    ...
    cursor.execute("""
        SELECT
            COUNT(*) as total,
            SUM(CASE WHEN shap_values IS NOT NULL AND shap_values != '{}' THEN 1 ELSE 0 END) as with_shap
        FROM prediction_log
        WHERE model_name LIKE '%LGBM%'
           OR model_name LIKE '%PatchTST%'
           OR model_name LIKE '%TimesNet%'
    ...
    logger.info(f"  Total ML model predictions: {stats[0]}")
    logger.info(f"  With SHAP values: {stats[1]}")
